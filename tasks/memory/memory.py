...
def relevant_for_injection(assistant_id: int, query: str, limit: int = 8) -> list:
    ...
    if len(clean) < 8:
        return [
            {**e, "relevance": "recent"}
            for e in recent_entries(assistant_id, recent_limit)
        ]
    try:
        hits = search_memory(
        ...
        hits = []
    bodies = entries_by_ids(assistant_id, [h["memoryId"] for h in hits])
    seen: set = set()
    merged: list = []
    for h in hits:
        ...
        merged.append(e)
        seen.add(e["id"])
    for r in recent_entries(assistant_id, recent_limit):
        if r["id"] in seen:
            continue
        r["relevance"] = "recent"
        merged.append(r)
