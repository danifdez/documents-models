...
Each processing slot owns an isolated, reusable handler process. Models polls control while work is active and terminates the complete handler process tree when Documents reports cancellation, the lease is rejected or the local lease window expires. The slot starts a clean replacement process before accepting more work. A healthy process stays alive between assignments so loaded handler state can be reused. Every assignment receives its own temporary workspace, which Models removes after success, failure or preemption.
